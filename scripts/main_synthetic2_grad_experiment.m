...
D_cutoff_gradinfeff = calc_gradinf_cutoff(k, d-1, n_vec, alpha);
...
hold on
scatter(feas_region(Sonlygrad_dinf_eff_indicators==1,1), feas_region(Sonlygrad_dinf_eff_indicators==1,2), 'r.')
scatter(exp_set(:,1), exp_set(:,2), 50, 'kx', 'LineWidth', 2) % plot experimental set X
scatter(1, 1, 50, 'k*', 'LineWidth', 2) % plot optimal solution
quiver(exp_set(:,1), exp_set(:,2), sample_mean_grad(:,1), sample_mean_grad(:,2), 0, 'b-', 'LineWidth', 1) % plot sample gradients
