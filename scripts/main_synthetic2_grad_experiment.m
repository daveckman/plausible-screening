...
scatter(feas_region(:,1), feas_region(:,2), 20, mean(Sonlygrad_dinf_eff_indicators,2)*rescale_coef, 'filled')
%scatter(feas_region(:,1), feas_region(:,2), 20, mean(Sonlygrad_dinf_eff_indicators,2), 'filled')
contour(x_mat, y_mat, true_mean_mat, 10, 'LineColor', 'k')
scatter(exp_set(:,1), exp_set(:,2), 50, 'kx', 'LineWidth', 2) % plot experimental set X
scatter(1, 1, 50, 'k*', 'LineWidth', 1) % plot optimal solution
