...
parfor m = 1:M
    ...
    [S_indicators_grad(:,m), D_x0s(:,m), S_poly_indicators_grad(:,m)] = GPS_screen(feas_region, exp_set, sample_mean, sample_mean_grad, sample_full_cov, n_vec, D_cutoff_grad);
...
true_grad = zeros(length(feas_region),1); % one dimensional problem
...
for i = 1:length(true_mean)
    ...
    true_grad(i) = cost - sell_price*(1-wblcdf(Q, wbl_scale, wbl_shape)) - salvage*wblcdf(Q, wbl_scale, wbl_shape) - shortage*(1-wblcdf(Q, wbl_scale, wbl_shape));
end
