function [outputs] = tandem_budget_oracle(oracle_rngs, solution, n_reps)
% Take n_reps replications at a given solution.
% Return the outputs as a row vector.
% Input solution is a row vector.

% budget_tandem_oracle simulates a tandem production line with manufacturing
...
% The objective is to choose the machine cycle times (subject to a
% constraint) that minimizes the expected completion time of the 100th
% product. See Plambeck et al. (1996) and Shanthikumar and Yao (1989) for 
% details.
...
buffers = [Inf, 4, 6, 8, 4]; % vector of buffers in front of each machine
