function [fn, FnVar, FnGrad, FnGradCov, constraint, ConstraintCov, ConstraintGrad, ConstraintGradCov] = SAN(x, runlength, seed, ~)
% function [fn, FnVar, FnGrad, FnGradCov, constraint, ConstraintCov, ConstraintGrad, ConstraintGradCov] = SAN(x, runlength, seed, other);
% x is a column vector equaling the decision variables theta
% runlength is the number of longest paths to simulate
...
if (runlength <= 0) || (round(runlength) ~= runlength) || (seed <= 0) || (round(seed) ~= seed),
    ...
    % Generate new stream
    DurationStream = RandStream.create('mrg32k3a');
    % Set the substream to the "seed"
    DurationStream.Substream = seed;
    % Set restore point for stream
    OldStream = RandStream.setGlobalStream(DurationStream);
    %% Generate random duration data
    RawExponentials = exprnd(1, runlength, numarcs);
    ColumnScaling = diag(theta);
    ArcDurations = RawExponentials * ColumnScaling;
