...
for x0_index = 1:length(scrX)
    ...
    p_x0(x0_index) = min(1, 1 - (1 - 2*beta_bar)^K); % since beta can only be in (0, 1/2);
...
scatter(scrX(:,1), scrX(:,2), [], p_x0, 'filled');
% for k = 1:size(scrX,1)
%     
...
%%
figure
stairs(sort(1-p_x0), 1:length(scrX), 'b', 'LineWidth', 2); %(1:length(scrX))/length(scrX)
set(gca, 'FontSize', 14, 'LineWidth', 2)
xlabel('$1-\alpha$', 'Interpreter', 'latex', 'FontSize', 16, 'FontWeight', 'bold')
ylabel('Number of Solutions in $\mathcal{S}$', 'Interpreter', 'latex', 'FontSize', 16, 'FontWeight', 'bold')
title('Sensitivity of Subset Size to $1-\alpha$', 'Interpreter', 'latex', 'FontSize', 16, 'FontWeight', 'bold')