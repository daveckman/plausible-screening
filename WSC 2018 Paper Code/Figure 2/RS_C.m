...
for kval = 1:length(kind)
    ...
    [Lv,xvvv(kval,:)] = QP2_sub(Ao,ao,Bo,bo);
...
for kval = 1:length(knind)
    ...
    if L(knind(kval)) < 0.75
        ...
        if ~isnan(t)
            smallerset = unique([ivsS(1:4) dt(t,:)]);
            ...
            for l = dt(t,:)
                ...
                lval = lval(indVV);
                sval = xvvv(l,(size(Bo,2)+size(Bo,1)+1):end)';
                sval = sval(indVV);
            ...
            Lv2 = QP2_sub(Aop,aop,Bop,bop,start_ind,-Offset2/2+LV/2);
        ...
        if L(knind(kval)) > 0.25
            ...
            if ~isnan(t)
                ...
                Lv = QP2_sub(Ao,ao,Bo,bo,start_ind,-Offset/2+LV/2);
            else
                for l = 1:size(xvvv,1)
                    lval = xvvv(l,(size(Bo,2)+1):(size(Bo,2)+size(Bo,1)))';
                    sval = xvvv(l,(size(Bo,2)+size(Bo,1)+1):end)';
